...
Select the `mimitag` scheme, your development team, and an iPhone/iPad target in Xcode. Build the Mac service first with Codex CLI signed in, run `agentd up`, then scan its short-lived pairing QR code in the app. For a command-line build check:
...
mimitag 是原生 iPhone / iPad SwiftUI 控制台。目录、Xcode 工程、target、scheme、Swift 模块和用户侧产品名统一使用 `mimitag`。旧版 `mimiremote://` 连接链接继续兼容，避免已有配对入口失效。
...
```bash
cd "$HOME/code/mimitag"
xcodegen generate --spec ios/mimitag/project.yml --project ios/mimitag
```
...
1. 用 Xcode 打开 `ios/mimitag/mimitag.xcodeproj`。
2. 选择 `mimitag` scheme。
