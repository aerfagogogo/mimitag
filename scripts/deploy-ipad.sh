...
PROJECT_PATH="${PROJECT_PATH:-$ROOT_DIR/ios/mimitag/mimitag.xcodeproj}"
SCHEME="${SCHEME:-mimitag}"
CONFIGURATION="${CONFIGURATION:-Debug}"
DEVICE_NAME="${DEVICE_NAME:-iPad Pro}"
DEVICE_ID="${DEVICE_ID:-}"
BUNDLE_ID="${BUNDLE_ID:-com.sunyiting.mimitag}"
TIMEOUT_SECONDS="${TIMEOUT_SECONDS:-120}"
DERIVED_DATA_PATH="${DERIVED_DATA_PATH:-$ROOT_DIR/ios/mimitag/build/deploy-derived}"
...
if [[ ! -d "$PROJECT_PATH" ]]; then
  ...
  echo "如果刚改过 project.yml，请先运行：xcodegen generate --spec ios/mimitag/project.yml --project ios/mimitag" >&2
  exit 1
fi
...
APP_PATH="$DERIVED_DATA_PATH/Build/Products/$CONFIGURATION-iphoneos/mimitag.app"
